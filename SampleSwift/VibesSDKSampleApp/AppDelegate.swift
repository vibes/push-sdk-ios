...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        let vibesConfig = VibesConfiguration(advertisingId: nil,
                           apiUrl: "<apiUrl>",
                           logger: self, // use `nil` if you dont wish to track SDK logs
...
extension AppDelegate: VibesLogger {
    
    func log(_ logObject: LogObject) {
        debugPrint("[TIMESTAMP] \(logObject.level.logPrefix) - \(logObject.message)")
        // Post the log to external 3rd party like DataDog
    }
    
    func log(error: Error) {
        debugPrint("[TIMESTAMP] \(error.localizedDescription)")
        // Post the log to external 3rd party like DataDog
    }
    
    func log(request: URLRequest) {
        debugPrint("[TIMESTAMP] \(request.url?.absoluteURL)")
        // Post the log to external 3rd party like DataDog
    }
