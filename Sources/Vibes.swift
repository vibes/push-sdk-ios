...
public class Vibes: EventTracker {
    ...
    public static let SDK_VERSION = "1.0.3"
    ...
    public func handleWalletPass(notification: [AnyHashable: Any], completion: @escaping ((PKAddPassesViewController?) -> ())) {
        if let clientData = notification["client_app_data"] as? [String: Any],
            let walletPkPass = clientData["wallet"] as? String{
            let task = URLSession.shared.dataTask(with: URL(string: walletPkPass)!) { (data, response, error) in
                if let data = data {
                    let pass = PKPass(data: data, error: nil)
                    let viewController = PKAddPassesViewController(passes: [pass])
