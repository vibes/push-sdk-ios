...
@objc public class Vibes: NSObject, EventTracker {
    ...
    public static let SDK_VERSION = "4.3.0"
            ...
            let registerDeviceCallback: VibesCompletion<VibesCredential> = { [weak self] result in
                ...
                case let .success(credential):
                    Vibes.shared.configuration.logger?
                        .log(LogObject(.info, message: "Silent Register Device Success: \(credential.deviceId)"))
                    self?.setPushToken(token: pushToken)
                    self?.registerPush()
                case let .failure(error):
                    Vibes.shared.configuration.logger?
                        .log(LogObject(.info, message: "Silent Register Device Failed: \(error)"))
                }
            }
    ...
    @objc public func getAppInfo(_ callback: GetVibesAppInfoCallback?) {
        self.submit(operation: GetVibesAppInfoOperation(credentials: self.credentialManager,
