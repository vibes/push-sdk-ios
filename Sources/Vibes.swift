...
@objc public class Vibes: NSObject, EventTracker {
  ...
  public static let SDK_VERSION = "4.2.0"
  ...
  @objc public func receivedPush(with userInfo: [AnyHashable: Any], at timestamp: Date = Date()) {
      ...
      if let pushToken = userInfo["vibes_auto_register_token"] as? String, // ensure push token is supplied
          ...
          let registerDeviceCallback: VibesCompletion<VibesCredential> = { [weak self] result in
              ...
              case let .success(credential):
                  print("Silent Register Device Success: \(credential.deviceId)") // we will replace this with SDK logger once implemented
                  self?.setPushToken(token: pushToken)
                  self?.registerPush()
              case let .failure(error):
                  print("Silent Register Device Failed: \(error)") // we will replace this with SDK logger once implemented
              }
          }
          ...
          print("Silent Register Device Failed: Valid Push Token was not supplied") // we will replace this with SDK logger once implemented
      }
  }
