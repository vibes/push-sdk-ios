...
class HTTPResourceClient: HTTPResourceClientType {
  ...
  func request<A>(resource: HTTPResource<A>, behavior: HTTPBehavior?, completion: HTTPCompletion<A>?) {
    ...
    let task = self.session.dataTask(with: request) { (data, response, error) in
      ...
      guard 200...299 ~= response.statusCode else {
        self.handleResponseWithStatusCode(of: response.statusCode, data: data, completion: completion)
        return
      }
  ...
  ///   - completion: A handler for when the request fails or succeeds.
  fileprivate func handleResponseWithStatusCode<A>(of statusCode: Int, data: Data?, completion: HTTPCompletion<A>?) {
