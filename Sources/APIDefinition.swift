...
enum APIDefinition {
  ...
  ///     record with Vibes
  static func registerDevice(appId: String, device: DeviceType) -> HTTPResource<Credential> {
    return jsonResource(
      path: "/\(appId)/devices",
      method: .POST,
      requestParameters: device.encodeJSON(),
      parse: Credential.init
    )
  }
  ...
  ///   - device: a `DeviceType` object with relevant device information to
  ///     record with Vibes
  static func updateDevice(appId: String, deviceId: String, device: DeviceType) -> HTTPResource<Credential> {
    return jsonResource(
